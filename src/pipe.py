...
def get_all_standard_pose(file):
    ...
    for i in range(total_frames):
        ...
        for joint in bvh_joint_list:
            joint_channels.append(mocap.frame_joint_channels(i - 1, joint, channels))
...
def get_key_pose(file, frame):
    ...
    for joint in bvh_joint_list:
        joint_channels.append(mocap.frame_joint_channels(frame - 1, joint, channels))
...
def main():
    ...
    while cap.isOpened():
        ...
        joint_group["mid_hip"] = get_center(
            joint_group["left_hip"], joint_group["right_hip"]
        ).tolist()
        joint_group["mid_shoulder"] = get_center(
            joint_group["left_shoulder"], joint_group["right_shoulder"]
        ).tolist()
        joint_group["spine"] = get_spine(
            joint_group["mid_shoulder"], joint_group["mid_hip"]
        ).tolist()
