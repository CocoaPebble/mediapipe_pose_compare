...
def angle_err(gt_angles, mp_angles, keypose=None):
    ...
    if keypose:
        ...
        for mp_frame in mp_angles:
            err = np.round(
                np.sum(np.abs(np.array(gt_frame) - np.array(mp_frame))), 3)
            all_err.append(err)
    else:
        frames = min(len(gt_angles), len(mp_angles))
        for frame in range(frames):
            err = np.round(
                np.sum(np.abs(np.array(gt_angles[frame]) - np.array(mp_angles[frame]))), 3)
            all_err.append(err)
    return all_err


def each_angle_err(gt_angles, mp_angles, keypose=-1, actual_min_pos=None, start=None, end=None):
    if keypose < -1:
        print('need keypose')
        raise TypeError
    if not start:
